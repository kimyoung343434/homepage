...
public class GuestDAO {
	...
	public boolean reply_create(GuestDTO dto){
		...
		sql.append("  values(  ");
		sql.append(" (select nvl (max(guestno),0)+1 from guest),   ");
		sql.append("   ?,?,?,?,sysdate,  ");
		sql.append("   ?,?,?,?)  ");
	...
	public List<GuestDTO> list(Map map){
		...
		StringBuffer sql = new StringBuffer();
		sql.append(" 	select guestno, wname, title, viewcnt, to_char(wdate, 'yyyy-mm-dd') wdate,  ");
	...
	public GuestDTO read(int guestno) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("   select guestno, wname, title, content, viewcnt, wdate, grpno  ");
		sql.append("  from guest   ");
		sql.append("  where guestno = ?   ");
